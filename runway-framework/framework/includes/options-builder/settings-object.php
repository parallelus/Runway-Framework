...
class Apm_Admin extends Runway_Admin_Object {
	...
	public function check_is_options_page_alias_unique( $page_alias ) {
		...
		foreach ( $pages as $own_page ) {
			if ( $page_alias != $own_page->settings->page_id && trim( $own_page->settings->alias ) == trim( $page_alias ) ) {
				$same_alias = true;
			}
		}
	...
	public function save_option_page() {
		...
		if ( isset( $json_page ) && $json_page != '' ) {
			...
			if ( $page_id ) {
				...
				if ( $same_alias ) {
					foreach ( $pages as $own_page ) {
						if (
							...
							&& preg_match( '/' . trim( $page['settings']['alias'] ) . '/', $own_page->settings->alias )
						) {
				...
				if ( $same_alias && count( $matches ) != 0 ) {
					$total = 0;
					foreach ( $matches as $alias ) {
						$tmp = array();
						if (
							$alias == $page['settings']['alias']
							|| ( preg_match( '/(.+)-[0-9+]/', $alias, $tmp ) && $tmp[1] == $page['settings']['alias'] )
						) {
				...
				foreach ( $new_elements as $element_key => $element_values ) {
					if (
						...
						// alias should not contain spaces
						$page['elements'][ $element_key ]['alias'] = preg_replace( '/\s+/', '',
							$page['elements'][ $element_key ]['alias'] );
					}
				}
					...
					if ( is_writable( $pages_dir ) ) {
						if ( isset( $_FILES['icon_url'] ) && file_exists( $_FILES['icon_url']['tmp_name'] ) ) {
							...
							if ( ! file_exists( $icons_dir ) && ! mkdir( $icons_dir, 2775, true ) ) {
								$message = '<div id="message" class="updated below-h2"><p>' .
								           __( 'I can not save icon', 'runway' ) . '</div>';
							}
						...
						if ( IS_CHILD && get_template() == 'runway-framework' ) {
							$wp_filesystem->put_contents(
								runway_prepare_path( "{$pages_dir}{$page_id}.json" ),
								$new,
								FS_CHMOD_FILE
							);
						}
						...
						$message = '<div id="message" class="updated below-h2"><p>' .
						           __( 'Page saved.', 'runway' ) . '</div>';
					} else {
						$message = '<p>' . sprintf(
								__( 'Saving error: %s must be a writable directory.', 'runway' ),
								$pages_dir
							) . '</p>';
					}
	 ...
	 */
	public function del_page( $page_id, $pages_dir ) {
	 ...
	 * @param string $pages_dir
	 */
	public function duplicate_page( $page_id, $pages_dir ) {
	...
	public function reset_to_default( $pages_dir, $page_id ) {
			...
			if ( isset( $elm->template ) && $elm->template == 'field' && isset( $elm->type ) ) {
				switch ( $elm->type ) {
					case 'range-slider-type':
						$json[ $elm->alias ] = ( isset( $elm->startSecondEntry ) && ! empty( $elm->startSecondEntry ) ) ?
							'[' . $elm->startFirstEntry . ', ' . $elm->startSecondEntry . ']' : '[' . $elm->startFirstEntry . ']';
						break;
					case 'font-select-type':
						$json[ $elm->alias ]['family'] = $elm->family;
						$json[ $elm->alias ]['weight'] = $elm->weight;
						$json[ $elm->alias ]['size']   = $elm->size;
						unset( $json[ $elm->alias ]['style'], $json[ $elm->alias ]['color'] );
						break;
					default:
	...
	public function get_pages_list() {
		...
		$error_flag    = true;
		$error_message = __(
			'<b>Error</b>: "data" and it\'s sub-folder "pages" must both exists and be writable. Please check these folders and their permissions in your child theme.',
			'runway'
		);
		...
		if ( $error_flag && ! is_writable( $this->pages_dir ) && ! is_writable( $this->data_dir ) ) {
		...
		foreach ( (array) $page['elements'] as $field_id => $element ) {
			if ( $field_id != 'none' /*&& $field_id > 0*/ && $field_id != 'n' ) {
				if ( isset( $page['elements'][ $field_id ]['title'] ) ) {
					$page['elements'][ $field_id ]['title'] = addslashes(
						htmlspecialchars( $element['title'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['values'] ) ) {
					$page['elements'][ $field_id ]['values'] = addslashes(
						htmlspecialchars( $element['values'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['requiredMessage'] ) ) {
					$page['elements'][ $field_id ]['requiredMessage'] = addslashes(
						htmlspecialchars( $element['requiredMessage'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['validationMessage'] ) ) {
					$page['elements'][ $field_id ]['validationMessage'] = addslashes(
						htmlspecialchars( $element['validationMessage'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['cssClass'] ) ) {
					$page['elements'][ $field_id ]['cssClass'] = addslashes(
						htmlspecialchars( $element['cssClass'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['titleCaption'] ) ) {
					$page['elements'][ $field_id ]['titleCaption'] = addslashes(
						htmlspecialchars( $element['titleCaption'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['fieldCaption'] ) ) {
					$page['elements'][ $field_id ]['fieldCaption'] = addslashes(
						htmlspecialchars( $element['fieldCaption'], ENT_QUOTES )
					);
				}
			}
		}
	...
	public function inputs_decode( $page ) {
		...
		foreach ( (array) $page['elements'] as $field_id => $element ) {
			if ( $field_id != 'none' /*&& $field_id > 0*/ && $field_id != 'n' ) {
				if ( isset( $page['elements'][ $field_id ]['title'] ) ) {
					$page['elements'][ $field_id ]['title'] = stripslashes(
						htmlspecialchars_decode( $element['title'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['values'] ) ) {
					$page['elements'][ $field_id ]['values'] = stripslashes(
						htmlspecialchars_decode( $element['values'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['requiredMessage'] ) ) {
					$page['elements'][ $field_id ]['requiredMessage'] = stripslashes(
						htmlspecialchars_decode( $element['requiredMessage'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['validationMessage'] ) ) {
					$page['elements'][ $field_id ]['validationMessage'] = stripslashes(
						htmlspecialchars_decode( $element['validationMessage'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['cssClass'] ) ) {
					$page['elements'][ $field_id ]['cssClass'] = stripslashes(
						htmlspecialchars_decode( $element['cssClass'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['titleCaption'] ) ) {
					$page['elements'][ $field_id ]['titleCaption'] = stripslashes(
						htmlspecialchars_decode( $element['titleCaption'], ENT_QUOTES )
					);
				}
				if ( isset( $page['elements'][ $field_id ]['fieldCaption'] ) ) {
					$page['elements'][ $field_id ]['fieldCaption'] = stripslashes(
						htmlspecialchars_decode( $element['fieldCaption'], ENT_QUOTES )
					);
				}
			}
		}
