	...
	if ( is_admin() ) {
		function title_button_edit_page( $title ) {
			...
			if ( $template == 'runway-framework' && $current ) {
				...
				$title .= ' <a href="'.network_admin_url('admin.php?page=options-builder&navigation=edit-page&page_id='. $current['id']) .'" title="'. __( 'Edit this page', 'framework' ) .'" class="add-new-h2">'. __( 'Edit Page', 'framework' ) .'</a>';
			}
			if ( IS_CHILD && $developerMode && $current ) {

				// Reset defaults
				$title .= ' <a href="'.network_admin_url('admin.php?page=options-builder&navigation=reset-fields-page&page_id='. $current['id']) .'" onclick="return confirm(\''. __( 'This will delete all saved settings on this page.\nAre you sure you want to to continue?', 'framework' ) .'\')" title="'. __( 'Reset all fields to defaults values.', 'framework' ) .'" class="add-new-h2">'. __( 'Reset Defaults', 'framework' ) .'</a>';
