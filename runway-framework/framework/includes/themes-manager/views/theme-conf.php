...
// Look up theme data
if ( ! isset( $options ) && isset( $_REQUEST['name'] ) ) {
...
$custom_icon_src = ( isset( $Folder ) && file_exists( get_home_path() . 'wp-content/themes/' . $Folder . '/custom-icon.png' ) ) ?
	home_url() . '/wp-content/themes/' . $Folder . '/custom-icon.png' :
	'';
...
<p>
	<?php echo __( 'Fill out the options below to create a new Runway child theme. The new child theme folder will be created in the', 'runway' ); ?>
	<code>wp-content/themes</code> <?php echo __( 'folder', 'runway' ); ?>.
</p>
...
<form method="post" enctype="multipart/form-data">
	...
	<?php if ( isset( $errors ) ) { ?>
		<ul class="errors"
		    style="border: solid 3px maroon; border-radius: 15px; width: 600px; padding: 10px; background-color: yellow;">
			<?php
	...
	<table class="form-table">
		...
		<?php
		$row = array(
			__( 'Title', 'runway' ) . $required,
			$html->settings_input( 'theme_options[Name]', isset( $Name ) ? $Name : '' )
		);
		...
		<tr class='choose-default-wordpress' style="display: none;">
			<td>
				<input class='dashicon-code-selected' name="theme_options[default-wordpress-icon-code]" type="hidden"
				       value=<?php echo isset( $options['default-wordpress-icon-code'] ) ? esc_attr( $options['default-wordpress-icon-code'] ) : ''; ?>>
				<input class='dashicon-class-selected' name="theme_options[default-wordpress-icon-class]" type="hidden"
				       value=<?php echo isset( $options['default-wordpress-icon-class'] ) ? esc_attr( $options['default-wordpress-icon-class'] ) : ''; ?>>
			</td>
			<td>
		...
		<tr class='custom-icon-upload' style="display: none;">
			...
			<td>
				...
				<p class="description">
					<?php echo __( 'Recommended size 36x36px for HiDPI screens. Icon displayed at 16x16px.', 'runway' ); ?>
				</p>
			</td>
		</tr>
		...
		$row     = array(
			__( 'Theme URI', 'runway' ),
			$html->settings_input( 'theme_options[URI]', isset( $URI ) ? $URI : '' ) . $comment
		);
		$html->setting_row( $row );

		$row = array(
			__( 'Description', 'runway' ),
			$html->settings_textarea( 'theme_options[Description]', isset( $Description ) ? $Description : '' )
		);
		$html->setting_row( $row );

		$row = array(
			__( 'Author name', 'runway' ),
			$html->settings_input( 'theme_options[AuthorName]', isset( $AuthorName ) ? $AuthorName : '' )
		);
		$html->setting_row( $row );
		...
		$comment = $html->format_comment( $comment );
		$row     = array(
			__( 'Author URI', 'runway' ),
			$html->settings_input( 'theme_options[AuthorURI]', isset( $AuthorURI ) ? $AuthorURI : '' ) . $comment
		);
		$html->setting_row( $row );

		$row = array(
			__( 'Version', 'runway' ),
			$html->settings_input( 'theme_options[Version]', isset( $Version ) ? $Version : '' )
		);
		$html->setting_row( $row );
		...
		$row     = array(
			__( 'Tags', 'runway' ),
			$html->settings_textarea( 'theme_options[Tags]', isset( $Tags ) ? $Tags : '' ) . $comment
		);
		$html->setting_row( $row );
		...
		$row     = array(
			__( 'Comments', 'runway' ),
			$html->settings_textarea( 'theme_options[Comments]', isset( $Comments ) ? $Comments : '' ) . $comment
		);
		$html->setting_row( $row );
		?>
		...
		<tr>
			...
			<td>
				<?php
				if ( isset( $Screenshot ) ) { ?>
					<a href="<?php echo esc_url( home_url() . '/wp-content/themes/' . $Folder . '/screenshot.png' ) ?>">
						<?php echo __( 'View Screenshot', 'runway' ); ?>
					</a>
					<br>
				<?php } ?>
				<input type="file" name="theme_options[Screenshot]"
				       value="<?php echo isset( $_FILES['theme_options']['name']['Screenshot'] ) ? esc_attr( $_FILES['theme_options']['name']['Screenshot'] ) : ''; ?>"/>
				<p class="description">
					<?php echo __( 'Recommended size: 600&#215;450 for HiDPI. Displayed at 300&#215;225.', 'runway' ); ?>
				</p>
			</td>
		</tr>
	...
	<div class="meta-box-sortables metabox-holder" style="width: 800px;">
		<div class="postbox">
			...
			<h3 class="hndle no-move"><span><?php echo __( 'Advanced', 'runway' ); ?></span></h3>
			<div class="inside" style="display: none;">
				...
				<table class="form-table">
					...
					$comment = $html->format_comment( $comment );
					$row     = array(
						__( 'Folder name', 'runway' ),
						$html->settings_input( 'theme_options[Folder]', isset( $Folder ) ? $Folder : '' ) . $comment
					);
					$html->setting_row( $row );
					...
					$row     = array(
						__( 'License', 'runway' ),
						$html->settings_input( 'theme_options[License]', isset( $License ) ? $License : '' ) . $comment
					);
					$html->setting_row( $row );
					...
					$comment = $html->format_comment( $comment );
					$row     = array(
						__( 'License URI', 'runway' ),
						$html->settings_input( 'theme_options[LicenseURI]',
							isset( $LicenseURI ) ? $LicenseURI : '' ) . $comment
					);
					$html->setting_row( $row );
					...
					$comment = $html->format_comment( $comment );
					$row     = array(
						__( 'Google API Key', 'runway' ),
						$html->settings_input( 'theme_options[WebFontAPIKey]',
							isset( $WebFontAPIKey ) ? $WebFontAPIKey : '' ) . $comment
					);
					$html->setting_row( $row );
	...
	<?php
	// Save button
	$submitText = ( $html->object->navigation == 'new-theme' ) ?
		__( 'Create Theme', 'runway' ) :
		__( 'Update', 'runway' );
