...
<div class="wp-filter">
	...
	<ul class="filter-links">
		...
		<li><a href="<?php echo admin_url('admin.php?page=directory&addons=extensions'); ?>" data-sort="extensions" class="<?php echo ($addons_type == 'extensions')? 'current' : ''; ?>"><?php echo __('Extensions', 'runway'); ?></a></li>
		<li><a href="<?php echo admin_url('admin.php?page=directory&addons=plugins'); ?>" data-sort="plugins" class="<?php echo ($addons_type == 'plugins')? 'current' : ''; ?>"><?php echo __('Plugins', 'runway'); ?></a></li>
		...
		<div class="theme-actions add-ons-init">
			<?php if( (isset($item_addons->isFree) && $item_addons->isFree) || (isset($item_addons->isPaid) && $item_addons->isPaid) ):
					switch ($addons_type) {
						case 'themes':
							if( in_array($key, $items_installed) ): ?>
								<?php if( isset($item_addons->Files) && count($item_addons->Files) ): ?>
									<a class="button button-primary add-ons-installed" data-key="<?php echo esc_attr($key); ?>" href="#"><?php echo __('Install', 'runway'); ?><div class="dashicons dashicons-arrow-down dashicons-position"></div></a>
									<div class="<?php echo esc_attr($key); ?>-installed-item add-ons-installed-menu" style="display:none">
										...
										foreach ($item_addons->Files as $file_key => $file_info) {
											...
											<?php if(!empty($file_info->package_id) && $file_info->package_id == 'child') : ?>
												<li><a href="<?php echo esc_url($download_link); ?>"><?php echo __('Install child theme', 'runway'); ?></a></li>
											<?php endif; ?>
											<?php if(!empty($file_info->package_id) && $file_info->package_id == 'standalone') : ?>
												<li><a href="<?php echo esc_url($download_link); ?>"><?php echo __('Install standalone theme', 'runway'); ?></a></li>
											<?php endif; ?>
											<?php if(!empty($file_info->package_id) && $file_info->package_id == 'full') :
												...
												<li><a href="<?php echo esc_url($download_link); ?>"><?php echo __('Download full theme package', 'runway'); ?></a></li>
											<?php endif; ?>
										<?php } ?>
										</ul>
									</div>
								<?php endif; ?>
							<?php else: ?>
								<a class="button button-secondary" href="<?php echo esc_url($item_addons->itemLink); ?>" target="_blank"><?php echo __('Details', 'runway'); ?></a>
							<?php endif;
						...
						case 'extensions':
							if( in_array($key, $items_installed) ): ?>
								<a class="button button-secondary" href="<?php echo esc_url($items_installed_link); ?>"><?php echo __('Installed', 'runway'); ?></a>
							<?php else: ?>
								<a class="button button-primary" href="<?php echo esc_url(admin_url('admin.php?page=directory&amp;addons=extensions&amp;action=install&amp;item='.$item_addons->Files[0]->name.'&amp;_wpnonce=')); ?>"><?php echo __('Install', 'runway'); ?></a>
							<?php endif;
							break;
